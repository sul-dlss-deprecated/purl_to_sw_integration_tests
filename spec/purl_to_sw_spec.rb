...
describe "All Purl objects released to SearchWorks" do
  ...
  it "SearchWorks has access_facet 'Online' for every Digital Collection object" do
    ...
    expect(resp).not_to include("id" => /.+/)  # get ids of errant records
  end
  ...
  it "correct number of digital collections" do
    ...
    # TODO:  when dispatcher service automagically tells SW to index newly released collection to SW prod,
    # these should match.
  ...
  it "released Purl collection objects are digital collections in SearchWorks" do
    ...
    purl_sw_colls_resp['response']['docs'].each do |doc|
      sw_doc_id =
        if doc['catkey_tsi']
          doc['catkey_tsi']
        else
          doc['id'].split('druid:').last
        end
      sw_doc_resp = solr_resp_single_doc(SolrConns.sw, sw_doc_id)
      sw_doc = sw_doc_resp['response']['docs'].first
      ...
      puts "ALERT: expected #{sw_doc_id} to be a Digital Collection!" unless sw_doc && sw_doc['collection_type'] == ['Digital Collection']
    end
    # TODO: using pending below causes jenkinsqa build to fail
    skip('when dispatcher service sends true releases to SW prod, we should use expect statement but output ids of probs')
  end
